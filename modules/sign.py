...
def get_args():
    agp = argparse.ArgumentParser(
        prog="sign.py", description="Gera um documento com assinatura digital."
    )

    agp.add_argument("file_path", help="Caminho para o arquivo a ser assinado.")
    agp.add_argument(
        "private_key_path", help="Caminho para o arquivo .pem com a chave privada."
    )
    agp.add_argument(
        "hash",
        choices=_hash_dict.keys(),
        default="sha512",
        help="Algoritmo de hash a ser usado.",
    )
    agp.add_argument(
        "-o", "--out_file", default="message", help="Arquivo de saída da assinatura."
    )
    agp.add_argument(
        "-v", "--verbose", action="store_true", help="Escreve no console os passos."
    )
